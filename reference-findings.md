# 参考页面核验记录

## 已确认的首屏与信息架构

参考页面并非传统的编辑式内容列表或落地页。其主体是一个占满整个视窗宽度的**纵向、无边框的漂浮信件档案**：暖白色背景上排列真实信笺形态的卡片，卡片大小、纸张颜色、旋转角度、水平位置和垂直间距均不规则。页面在首屏不展示标题、导航、说明区或页脚。

左上角有一枚独立、固定的竖向椭圆蜡封；它是唯一常驻界面元素。页面源码显示，蜡封正反两面可翻转，背面包含一段介绍文本。点击每封信进入阅读模式。正文流总高度约 22,894 px，以纵向滚动持续浏览。

## 已确认的首屏构图（1440 × 1000）

页面背景为接近白色的暖灰。左上角蜡封尺寸约 60 px × 96 px。首屏中的信件至少三封：左上是窄幅浅米色手写信，中右是大幅金黄色手写信，下左有一封较大的白色信。它们不对齐网格，保留大量空白。信件本身以高对比度深色手写体呈现，纸张有真实边缘和多页轻微错位。

## 移动端与蜡封开启状态

在 375 px 宽的移动端，固定蜡封仍位于左上，尺寸约为 60 px × 96 px。信件从顶部开始改为单列纵向流：首封浅米色信件宽约 224 px，第二封白色信件宽约 300 px，左右以温和的错位取代桌面端的大跨度散点。背景和卡片质感保持不变。

蜡封被点击后，页面的信件背景会整体淡化，中央出现约 240 px × 374 px 的金色竖向椭圆面。该椭圆承载简短说明文字，内容为深褐色衬线体；背景没有明显的黑色蒙层，仅保留极轻的失焦和降亮度。再次点击背景或蜡封可收起说明并恢复档案流。

## 滚动与信件阅读器取证（待继续补齐关闭状态）

参考站的档案不是普通页面滚动，而是固定在全屏内、隐藏滚动条的 `vertical-viewport` 独立滚动容器。滚动到 1100 px 时，信件仍以各自不同的 `translateY` 视差值移动；已采样的六张卡片分别出现约 `-25.6`、`+2.0`、`-16.2`、`+0.2`、`+2.8`、`-7.7` px 的额外纵向偏移。这意味着后续需要使用每张信独立的视差速率，不能只让整个轨道整体移动。

点击信件后，参考站会给滚动容器添加 `has-selection` 并隐藏档案操作，随后出现独立的 `reader-backdrop is-webgl-focused` 阅读器。桌面阅读器包含左侧的大幅放大信纸，以及右侧的作者、背景、日期、地点、来源元数据栏；移动端另有独立关闭按钮。关闭入口是 `reader-dismiss-layer`，而非普通弹层的遮罩或蜡封背景。此前取证会话因已打开蜡封而污染了滚动与阅读截图，因此关闭后的恢复状态尚需在“蜡封关闭、阅读器关闭”的干净会话中再次捕获。

在干净会话中，点击位于 `scrollTop = 1100 px` 的信件后，滚动容器类名从 `vertical-viewport` 切换为 `vertical-viewport has-selection is-intro-locked`，但 `scrollTop` 保持 `1100 px`。阅读器的独立舞台宽约 1050 px，并由覆盖整个视窗的 `reader-dismiss-layer` 承担关闭操作。关闭后，阅读器 DOM 被移除，滚动容器恢复为 `vertical-viewport`，`scrollTop` 仍准确保持 `1100 px`，原信件继续显示在此前的可见位置。

桌面阅读器的已确认静态结构是：左侧为接近 640 px 宽的放大原信纸，右侧为约 320 px 宽的元数据栏。标题为大号衬线体；下方依次是简介分隔线、Date、Place 和 Source。截图中放大信纸曾因图形层资源延迟而短暂空白，但读者容器及元数据栏的结构和尺寸已被量化确认。

## 本地实现的交互验证结果

本地验证会话在 `scrollTop = 1100 px` 时确认，六张信件的矩阵平移分别包含 `-79`、`+63`、`-33`、`+77`、`-52`、`+23` px 的独立 Y 轴偏移；这证明当前实现不是单纯整轨滚动，而是已启用逐封信的差异化视差。点击可见信件后，阅读器以 `reader-backdrop is-active` 进入，档案添加 `has-selection`，阅读舞台尺寸为 1050 × 642 px，呈现左纸右信息布局。点击覆盖整个视窗的关闭层后，阅读器移除、档案恢复正常状态且 `scrollTop` 仍为精确的 1100 px。

## 公开设置采集：桌面端

公开采集确认，参考页面由 3 份样式表、12 个前端脚本、至少 60 条信纸图资源路径构成；这些原始资产均只登记路径，不会下载或复用。当前 1440 × 857 桌面端中，画布的实际背景色为 `rgb(247, 247, 245)`，轨道实际宽度为 1152 px、高度为 23,191.5 px，等于视窗宽度的 80%。蜡封元素原始尺寸为 200 × 200 px，以 `scale(.5)` 显示为约 100 × 100 px，固定 `top:18px`、`left:-13px`，变换原点为 `36px 4px`。

逐信公开变量已完整采集。前六封信的桌面 `data-parallax-rate` 分别为 `-0.02`、`0.0033`、`-0.0167`、`0.0067`、`-0.0133`、`0.01`；其纸张宽度节奏为 224、356、510、268、438、196 px。参考站真实轨道比当前项目的 10,060 px 长得多，后续若追求全站空间节奏，必须将当前信件数量、轨道高度、X 坐标与宽度循环扩展到采集数据的规模。

## 公开设置采集：真实移动断点与动态状态

经过设备模拟，已确认真实移动 CSS 视口为 375 × 812 px。参考站在此断点将轨道变为 375 px 宽、24,526 px 高的流式容器，内边距为 `1px 18px 120px`；卡片从绝对定位变为相对定位，第一张卡片实际位于 `x:47.5px`、`y:136.65px`、宽 224 px，第二张卡片位于 `x:68px`、`y:574.45px`、宽 295 px。由于窄屏规则，超宽信纸会收缩到 295 px（视口减去 80 px），并以约 ±28 px 的横向偏移交错排列。

解除初始锁定后，蜡封开启状态的数据也已确认：类名变为 `archive-seal is-open`，显示尺寸为 400 × 400 px，位置为 `x:520px`、`y:228.5px`，基础变换为 `scale(2)`；背景层类名为 `archive-seal-backdrop is-visible`，不透明度为 1，背景为 `rgba(247,247,245,.72)`，过渡时长为 0.5 秒。桌面阅读器的精确网格为 `640px 320px`、间隙 56 px、舞台宽 1050 px、上下内边距为 56 px 与 48 px；关闭后实际保留 `scrollTop = 1100 px`。

## 公开源码中确认的交互与实现线索

页面使用 `vertical-archive`、`vertical-viewport`、`vertical-track` 和 `vertical-letter-slot` 组成独立滚动轨道。信件按 `--letter-width`、`--letter-x`、`--letter-y`、`--letter-gap` 和 `--letter-offset` 自定义定位，并带不同 `data-parallax-rate`。每一封信都是可点击按钮，内部是可展开的多页信笺堆叠。信笺样式包含 rag-paper、plain-ivory、service-blue、close-written-fold、office-typewriter、correspondence-card、ruled-notebook 与 red-folded 等变体。

## 复刻边界

后续实现将严格复刻已确认的布局逻辑、空间节奏、滚动与展开模式，但不会引用、下载或发布参考站的蜡封图、书信图、原始文案、字体文件或源代码。用户的原创表达将被渲染为自有纸张卡片，并使用新生成的原创视觉资产。
